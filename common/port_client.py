...
class Simple_Client:
    ...
    def __init__(self, host, port):
        # tracks the port that this client's server is using
        self.my_svr_port = port
        # tracks the host that tis client is on
        self.my_svr_host = host
    ...
    def iterative_broadcast(self, host, min_port, random_offset, port_range, msg):
        ...
        while loops > 0:  # should be a timeout
            ...
            for p in range(0, port_range):
                ...
                if not ((port == self.my_svr_port) and (host == self.my_svr_host)):
    ...
    # Mode 2 - attempt connection, can retry for # of attempts which is
    # default to 1
    def attempt_to_connect(self, host, port, msg, attempts=1):
        ...
        while (not isConnected) and (connect_attempts > 0):
            # print(connect_attempts),
            connect_attempts = connect_attempts - 1
            try:
                # blocking function that uses a socket to send and receive the
                # message
                reply = self.connect_send__receive_close(host, port, msg)
                time.sleep(0.01)
                # if the send and receive function completed, we assume the
                # message exchange occured
